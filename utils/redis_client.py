...
class RedisClient:
    ...
    def __init__(self, host='localhost', port=6379, db=0, password=''):
        ...
        :param password: 密码
        """
        pool = redis.ConnectionPool(host=host, port=port, db=db, password=password, max_connections=None)
    ...
    def __new__(cls, *args, **kwargs):
        if not hasattr(RedisClient, "_instance"):
            with RedisClient._instance_lock:
                RedisClient._instance = super(RedisClient, cls).__new__(cls)
    ...
    def set_expire_by_second(self, key, second=60 * 60 * 24 * 7):
        """
            以秒为单位设置过期时间
        :param key: 键
        :param second: 默认7天
    ...
    def get_expire_by_second(self, key):
        """
            以秒为单位返回key的剩余过期时间
        :param key: 键
        :return:
    ...
    def add_str(self, key: str, value: str):
        ...
        self.logger.debug(f"[Redis] Successfully added <{key}:{value}>") \
            if result else self.logger.error(f"[Redis] Failed added <{key}:{value}>")
        return result
    ...
    def append_str(self, key, append_str: str):
        """
    ...
    def add_list(self, key, direction=0, *value):
        """
            列表添加数据
        :param key: 键
        :param value: 列表值
    ...
    def pop_list_by_direction(self, key, direction=0):
        """
            根据删除方向删除列表数据
        :param key: 键
    ...
    def pop_list_by_value(self, key, value, count=1):
        """
            根据指定列表值删除列表数据
        :param key: 键
        :param value:指定列表值
    ...
    def add_hash(self, key, field, value):
        """
            哈希添加数据
        :param key: 键
        :param field: filed域
    ...
    def get_hash_by_field(self, key, field):
        """
    ...
    def get_hash_all_filed(self, key):
        """
            获取所有的field值
        :param key: 键
    ...
    def get_hash_all_value(self, key):
        """
            获取所有的value值
        :param key: 键
    ...
    def get_hash_all_KeyValue(self, key):
        """
    ...
    def delete_set_random(self, key):
        """
            移除指定key的随机value值
        :param key: 键
        :return: 删除的值
    ...
    def delete_set_assign(self, key, *values):
        """
            移除指定key的指定value值
        :param key: 键
    ...
    def add_sortSet(self, key, mapping):
        """
    ...
    def get_sortSet_by_indexRange(self, key, start=0, end=-1, with_score=True):
        """
    ...
    def get_sortSet_by_scoreRange(self, key, min, max):
        """
    ...
    def delete_sortSet_by_indexRange(self, key, start=0, end=-1):
        """
    ...
    def delete_sortSet_by_scoreRange(self, key, min, max):
        """
